...
// Copyright (c) 2013-2020 The Curecoin developers
...
unsigned int nStakeTargetSpacing = 1 * 600; // 600 seconds block spacing
const int64 nChainStartTime = 1398151422;
            ...
            if (nHeight == 1){
                    ...
                    nSubsidy = 6.5 * COIN;
                     ...
                     nSubsidy = 3.25 * COIN;
                      ...
                      nSubsidy = 0.8125 * COIN;
            }
			else if (nHeight < 1261440){
                    nSubsidy = 0.40625 * COIN;
            }
            else if (nHeight < 1471680){
                    nSubsidy = 0.203125 * COIN;
            }
            else if (nHeight < 1681920){
                     nSubsidy = 0.101562 * COIN;
            }
			else if (nHeight < 1892160){
                      nSubsidy = 0.050781 * COIN;
            }
			else if (nHeight < 2102400){
                      nSubsidy = 0.025390 * COIN;
            }
			else if (nHeight < 2312640){
                    nSubsidy = 0.012695 * COIN;
            }
            else if (nHeight < 2522880){
                    nSubsidy = 0.006347 * COIN;
            }
            else if (nHeight < 2733120){
                     nSubsidy = 0.003173 * COIN;
            }
			else if (nHeight < 2943360){
                      nSubsidy = 0.001586 * COIN;
            }
			else if (nHeight < 3153600){
                      nSubsidy = 0.000793 * COIN;
            }
			else if (nHeight < 3363840){
                    nSubsidy = 0.000396 * COIN;
            }
            else if (nHeight < 3574080){
                    nSubsidy = 0.000198 * COIN;
            }
            else if (nHeight < 3784320){
                     nSubsidy = 0.000099 * COIN;
            }
			else if (nHeight < 3994560){
                      nSubsidy = 0.000049 * COIN;
            }
			else if (nHeight < 4204800){
                      nSubsidy = 0.000024 * COIN;
            }
			else if (nHeight < 4415040){
                    nSubsidy = 0.000012 * COIN;
            }
            else if (nHeight < 4625280){
                    nSubsidy = 0.000006 * COIN;
            }
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    if (nBestHeight > (int)HF_BLOCK) bnRewardCoinYearLimit = (int64)(0.04 * MAX_MINT_PROOF_OF_WORK); // 4% hardfork
    CBigNum bnTarget;
    bnTarget.SetCompact(nBits);
    CBigNum bnTargetLimit = bnProofOfStakeLimit;
    bnTargetLimit.SetCompact(bnTargetLimit.GetCompact());

    // curecoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                               ...
                               bool fBlock, bool fMiner, MapPrevTx& inputsRet, bool& fInvalid)
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        if (prevout.n >= txPrev.vout.size() || prevout.n >= txindex.vSpent.size())
        {
            ...
            // adding inputs:
            fInvalid = true;
            return DoS(100, error("FetchInputs() : %s prevout.n out of range %d %zu %zu prev tx %s\n%s", GetHash().ToString().substr(0,10).c_str(), prevout.n, txPrev.vout.size(), txindex.vSpent.size(), prevout.hash.ToString().substr(0,10).c_str(), txPrev.ToString().c_str()));
        }
    }
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal curecoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // curecoin: fees are not collected by miners as in ppcoin/bitcoin
    // curecoin: fees are destroyed to compensate the entire network
    // NOTE: print stuff is moved above writing changes for fJustCheck
    if (fDebug && GetBoolArg("-printcreation"))
...
static void runCommand(const std::string& strCommand)
{
    std::cout << std::flush;
    int ret = std::system(strCommand.c_str());
    // note: return value is implementation defined, but unix and windows seem to both use return value of caled program
    if (ret != 0)
    {
        printf("potential runCommand error: system(%s) returned %d\n", strCommand.c_str(), nErr);
    }
}
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // curecoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // curecoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // curecoin: record proof-of-stake hash value
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016llx", pindexNew->nHeight, nStakeModifier);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // curecoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // curecoin: coinbase output should be empty if proof-of-stake block
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        // curecoin: check transaction timestamp
    ...
    // curecoin: check block signature
...
bool CBlock::SignBlock(const CKeyStore& keystore)
{
    ...
    const bool is_PoS = IsProofOfStake();
    // TODO: what is this vtx[0].vout.size() about?
    for (unsigned int i = 0; i < (is_PoS ? 1 : vtx[0].vout.size()); i++)
    {
        const CTxOut& txout = is_PoS ? vtx[1].vout[1] : vtx[0].vout[i];
...
bool CBlock::CheckBlockSignature() const
{
    ...
    const bool is_PoS = IsProofOfStake();
    // TODO: again, what is vtx[0].vout.size() about?
    for (unsigned int i = 0; i < (is_PoS ? 1 : vtx[0].vout.size()); i++)
    {
        const CTxOut& txout = is_PoS ? vtx[1].vout[1] : vtx[0].vout[i];;
...
// TODO: ppcoin has GetStakeEntropyBit
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "CureCoin", (fTestNet ? hashGenesisBlockTestNet : hashGenesisBlock).ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "This Blind Man Can Show You How To Look On The Bright Side.";
        CTransaction txNew;
        ...
        if (IsCalculatingGenesisBlockHash && (block.GetHash() != hashGenesisBlock)) {
            ...
            while (block.GetHash() > hashTarget)
            {
                ...
                // lower 13 bits are zero, i.e. divisible by 16384
                // should be more efficient than modulo 10
                if ((block.nNonce & ((1 << 14) - 1)) == 0)
				{
					printf("nonce %08x, time %u: hash = %s \n", block.nNonce, block.nTime, block.GetHash().ToString().c_str());
				}
            }
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x149cd359a5ea486d50f10b4cef2a627d1890cb79eacea2eb36ce2dafec2fffbe"));
        block.print();
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%u,%u) %s  %08x  %s  mint %7s  tx %zu",
               pindex->nHeight,
               pindex->nFile,
               pindex->nBlockPos,
               block.GetHash().ToString().c_str(),
...
// TODO: ppcoin has CAlert::Processalert() -> bool here
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv)
{
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        std::vector<CInv> vInv;
                        // TODO: ppcoin has GetLastBlockIndex(pindexBest, false)->GetBlockHash() instead of hashBestChain
    ...
    else if (strCommand == "alert")
    {
        ...
        if (pfrom->setKnown.contains(alertHash))
        {
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (true)
    {
        ...
        if (pstart - vRecv.begin() > 0)
            printf("\n\nPROCESSMESSAGE SKIPPED %td BYTES\n\n", pstart - vRecv.begin());
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        std::vector<CInv> vInvWait;
        {
            ...
            {
                if (pto->setInventoryKnown.contains(inv))
                    continue;
...
public:
    ...
    COrphan(CTransaction* ptxIn) : ptx(ptxIn) {}
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // this was added in 0.6.0?? I suppose I'll keep it
    ...
    int64 nFees = 0;
    {
        ...
        for (std::map<uint256, CTransaction>::iterator mi = mempool.mapTx.begin(); mi != mempool.mapTx.end(); ++mi)
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin)
            {
                ...
                if (!txPrev.ReadFromDisk(txdb, txin.prevout, txindex))
                {
                    const auto it = mempool.mapTx.find(txin.prevout.hash);
                    ...
                    nTotalIn += (*it).vout[txin.prevout.n].nValue;
                    continue;
                }
                int64 nValueIn = txPrev.vout[txin.prevout.n].nValue;
                nTotalIn += nValueIn;
            ...
            if (porphan)
            {
                ...
                vecPriority.emplace_back(dPriority, dFeePerKb, &(*mi).second);
        }
            ...
            if (mapDependers.count(hash))
            {
                BOOST_FOREACH(COrphan* porphan, mapDependers[hash])
                {
                    if (!porphan->setDependsOn.empty())
                    {
                        ...
                        if (porphan->setDependsOn.empty())
                        {
                            vecPriority.emplace_back(porphan->dPriority, porphan->dFeePerKb, porphan->ptx);
        ...
        // TODO: ppcoin has pblock->hashMerkleRoot = pblock->BuildMerkleTree();
        if (pblock->IsProofOfStake())
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("curecoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("curecoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("curecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("curecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadcurecoinMiner(void* parg);

static bool fGeneratecurecoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void curecoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("curecoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratecurecoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratecurecoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running curecoinMiner with %zu transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadcurecoinMiner(void* parg)
{
    ...
    try
    {
        ...
        curecoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadcurecoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadcurecoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadcurecoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatecurecoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratecurecoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratecurecoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d curecoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadcurecoinMiner, pwallet))
                printf("Error: NewThread(ThreadcurecoinMiner) failed\n");
            Sleep(10);
        }
    }
}
